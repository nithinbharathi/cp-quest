...
public class Problem {
	...
	public static void main(String[] args) {
		...
		while (test-- > 0) {
			...
			long scoreB = go(pb, n,k, a,p);
			long scoreS = go(ps,n,k,a,p);
			sb.append(scoreB>scoreS?"Bodya\n":scoreS>scoreB?"Sasha\n":"Draw\n");
		}
	...
	private static long go(int pos, int n, long k, int a[], int p[]) {
		long score = 0;
		...
		while(!vis[pos] && k>0) {
			...
			sum+=a[pos];
			k--;
			score = Math.max(score, sum+a[pos]*k);			
			pos = p[pos];
