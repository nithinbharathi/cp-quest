	...
	public static void main(String[] args) {
		 ...
		 while(test-->0) {
			 ...
			 for(int i = 0;i<Math.min(m/2,n/2);i++) {
				 ...
				 for(int j =i+1;j<endR;j++) {
					l.add(c[j][endC]);
				 }
				 for(int j =endC;j>=i;j--) {
					l.add(c[endR][j]);
				 }
				 
				 for(int j =endR-1;j>i;j--) {
					l.add(c[j][i]);
				 }
				 ...
				 for(int j = 0;j<l.size();j++) {					 
					 ...
					 int ind = j;
					 for(int k = 0;k<4;k++) {
						 int curVal = l.get(ind++);
						 if(curVal == '1')
							 cur = 1;
						 else if(curVal == '5' && prevVal == '1')
							 cur++;
						 else if(curVal == '4' && prevVal == '5')
							 cur++;
						 else if(curVal == '3' && prevVal == '4')
							 cur++;
						 else 
							 cur = 0;
