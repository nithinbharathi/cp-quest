...
public class Problem {
		...
		public static void main(String[] args) {
			...
			while(test-->0) {
				...
				for(int j = 0;j<n;j++)b[j] = t.readInt();
				...
				for(Map.Entry<Integer,Integer>mp:map.entrySet()) {
					long key = mp.getKey();
					long value = mp.getValue();
					
					long k1 = Math.min(value, m/key);
					long rem = m-k1*key;
					
					ans = Math.max(ans, k1*key);
					
					int keyy = (int)(key+1);
					if(map.containsKey(keyy)) {
						long k2 = Math.min(map.get(keyy), rem/keyy);
						rem = rem-k2*keyy;
						long min = Math.min(Math.min(k1, map.get(keyy)-k2),rem);
						ans = Math.max(ans, (k1-min)*key + (k2+min)*keyy);				
					}
				}
