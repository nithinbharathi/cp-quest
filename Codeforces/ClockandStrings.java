...
public class Problem {
	...
	public static void main(String[] args) {
		...
		while (test-- > 0) {
			...
			sb.append( (Math.max(a, b) < Math.min(c, d) || Math.max(c, d) < Math.min(a, b) || overlap(a,b,c,d))?"NO\n":"YES\n");
		}
	...
	private static boolean overlap(int a, int b, int c ,int d) {
		return (Math.min(a, b)<=Math.min(c, d) && Math.max(c, d)<=Math.max(a, b))
			|| (Math.min(c, d)<=Math.min(a, b) && Math.max(a, b)<=Math.max(c, d));
	}
