	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			for(int i = 0;i<26;i++){
				...
				int cur = -1;
				for(int j = 0;j<len;j++){
					int blk_size = cnt[i].get(j)-st-1;
					st = cnt[i].get(j);
					
					cur = Math.max(log(blk_size), cur);
					if(j == len-1){
						blk_size = (n-cnt[i].get(j)-1);
						cur = Math.max(log(blk_size), cur);
					}
