...
/**
 * @submission : Nithin Bharathi 01-July-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			boolean ok = false;
			for(int i  =0;i<m;i++){
				String first = tar.substring(0,i+1);
				String second = tar.substring(i+1,m);
				if(have(first,new StringBuilder(second).reverse().toString(),s)){ok = true;break;}
			}
	...
	public static boolean have(String first,String sec,String s){
		int n = s.length();
		for(int i = 0;i+first.length()<=n;i++){
			String sub = s.substring(i,i+first.length());
			if(first.equals(sub) && (i+first.length()-1)-sec.length()>=0){
				String sub2 = s.substring((i+first.length()-1)-sec.length(),i+first.length()-1);
				if(sec.equals(sub2))return true;
			}
		}
		return false;
	}
