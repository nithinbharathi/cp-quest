...
/**
 * @submission : Nithin Bharathi 26-Aug-2023
 *
 *
 */
...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++){
				a[i] = t.readInt();
				if(a[i]>0)pos = a[i]>a[pos]?i:pos;
				if(a[i]<0)neg = a[i]<a[neg]?i:neg;
			}
			ArrayList<Pair>l = new ArrayList<>();
			if(a[pos]<Math.abs(a[neg])){
				for(int i = 0;i<n;i++){
					if(a[i]>0)l.add(new Pair(i+1,neg+1));
				}
				for(int i = n-2;i>=0;i--)l.add(new Pair(i+1,i+2));
			}else{
				for(int i = 0;i<n;i++){
					if(a[i]<0)l.add(new Pair(i+1,pos+1));
				}
				for(int i = 1;i<n;i++)l.add(new Pair(i+1,i));
			}
			sb.append(l.size()+"\n");
