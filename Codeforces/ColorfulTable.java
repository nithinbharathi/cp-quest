	...
	public static void main(String[] args) throws Exception {
		...
		Template t = new Template();
		 ...
		 while(test-->0) {
			 ...
			 Arrays.fill(first,Integer.MAX_VALUE);Arrays.fill(last,Integer.MIN_VALUE);
			 for(int i  =0;i<n;i++)a[i] = t.readInt();
			 int min = n,mx = -1;
			 for(int i = 0;i<n;i++) first[a[i]] = Math.min(i,first[a[i]]);
			 for(int i = 0;i<n;i++) last[a[i]] = Math.max(i,last[a[i]]);

			 for(int i =k;i>=0;i--) {
				 if(first[i] != Integer.MAX_VALUE) {
					 first[i] = min = Math.min(min,first[i]);
					 last[i] = mx = Math.max(mx,last[i]);
				 }
			 }			
			
			 for(int i = 1;i<=k;i++) {
				 if(first[i] == Integer.MAX_VALUE)sb.append(0);
				 else sb.append((last[i]-first[i]+1)*2);
				 sb.append(" ");
			 }
			 sb.append("\n");
		 }
