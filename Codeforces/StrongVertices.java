...
/**
 * @submission : Nithin Bharathi 16-Aug-2023
 *
 *
 */
...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++)l.add(new Pair(a[i]-b[i],i+1));
			Collections.sort(l,(p1, p2)->{
				return (p1.x>p2.x)?1:((p1.x == p2.x)?0:-1);
			});
			ArrayList<Integer>res = new ArrayList<>();
			for(int i = l.size()-1;i>=0;i--){
				long left = go(l,l.get(i).x,i);
				long ans=i;
				if(left != -1)ans+=(left-i);
				if(ans == n-1)res.add(l.get(i).y);
			}
	...
	public static int go(ArrayList<Pair>li, int tar,int l){
		int r = li.size()-1;
		int ind = -1;
		while(l<=r){
			int mid = (l+r)>>1;
