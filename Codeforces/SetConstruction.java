...
/**
 * @submission : Nithin Bharathi 2-Sept-2023
 *
 *
 */
...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			...
			for(int i = 0;i<n;i++) {
				c[i] = t.read().toCharArray();
				set.add(i);
			}
			HashSet<Integer>res[] = new HashSet[n];
			for(int i = 0;i<n;i++)res[i] = new HashSet<>();
			for(int i = 0;i<n;i++) {
				for(int j = 0;j<n;j++) {
					if(c[i][j] == '1') {
						graph[j].add(i);
			...
			for(int it:set)go(it,res);
			
			for(int i= 0;i<n;i++) {
				sb.append(res[i].size()+" ");
				for(int it:res[i])sb.append(it+" ");
				sb.append("\n");
			}
		}
		System.out.println(sb);
	}
	public static void go(int v,HashSet<Integer>res[]) {
		vis[v] = true;
		for(int child:graph[v]) {
			if(!vis[child])go(child,res);
			for(int it:res[child])res[v].add(it);
