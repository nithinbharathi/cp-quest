...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			...
			for(int i = 0;i<n;i++)a.add(new Pair(t.readInt(),i));
			for(int i = 0;i<n;i++)b.add(new Pair(t.readInt(),i));
			for(int i = 0;i<n;i++)c.add(new Pair(t.readInt(),i));
			...
			for(int i = n-3;i<n;i++) {
				for(int j = n-3;j<n;j++) {
					for(int k = n-3;k<n;k++) {
						if(a.get(i).y != b.get(j).y && b.get(j).y  != c.get(k).y && a.get(i).y != c.get(k).y ) {
							ans = Math.max(ans,a.get(i).x + b.get(j).x + c.get(k).x);
						}
					}
				}
			}
