...
public class Problem {
	...
	public static void main(String[] args) {
		...
		while(test-->0) {
			int n = t.readInt(), a = t.readInt(), b = t.readInt(), c = t.readInt();
			
			int tn = n;
			int ans = 0;
			int min = Math.min(tn, a); ans += min;
			tn-=min;
			min = Math.min(c, tn); ans += min;
			c-=min;
			
			tn = n;
			min = Math.min(tn, b); ans+= min;
			tn-=min;
			min = Math.min(tn, c); ans+= min;
