...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++)a.add(t.readInt());
			for(int i = 0;i<n;i++)b.add(t.readInt());
			cnt = 0;
			while(!a.isEmpty()) {
				int first = a.remove();
				int second = b.remove();
				if(first != second) {
					if(first>second) {
						...
						int len = String.valueOf(second).length();
						b.add(len);
						a.add(first);
					}
					cnt++;
	...
	public static int go(ArrayList<Pair>pos, long k,int n) {
		int ind = -1;
		long diff = 0;
