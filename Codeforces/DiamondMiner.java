...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			ArrayList<Double>miner = new ArrayList<>();
			for(int i = 0;i<2*n;i++){
				double a = t.readInt();
				double b = t.readInt();
				if(a == 0)miner.add(Math.abs(b));
				else mine.add(Math.abs(a));
			}
			Collections.sort(mine);Collections.sort(miner);
			double ans = 0;
			for(int i = 0;i<n;i++){
				double minerDis = miner.get(i)*miner.get(i);
				double mineDis = mine.get(i)*mine.get(i);
				double dis = Math.sqrt(minerDis+mineDis);
				ans+=dis;
			}
				sb.append(ans+"\n");
	...
	public static boolean check(char symbol,char c[][]){
		int n = 3;
		boolean row = true,col = true,diagonal = true;
		for(int i = 0;i<n;i++){
			diagonal&=(c[i][i] == symbol);
		}
		if(diagonal)return true;
		diagonal = true;
		for(int i = 0;i<n;i++){
			diagonal&=(c[i][n-i-1] == symbol);
		}
		if(diagonal)return true;
