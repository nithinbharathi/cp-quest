		...
		public static void main(String[] args) {
			...
			while(test-->0) {
				...
				for(int i = 0;i<m;i++) {
					String s = t.read();
					boolean ok = true;
					HashMap<Integer,Character>atos = new HashMap<>();
					HashMap<Character,Integer>stoa = new HashMap<>();
					if(s.length() == n) {
						for(int j = 0;j<n;j++) {
							if(!atos.containsKey(a[j])) {
								if(!stoa.containsKey(s.charAt(j))) {
									stoa.put(s.charAt(j), a[j]);
									atos.put(a[j], s.charAt(j));
								}else {
									ok = false;
									break;
								}
							}else {
								ok &= atos.get(a[j]) == s.charAt(j);
							}
						}
						sb.append(ok?"YES\n":"NO\n");
					}else {
