...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			HashSet<Integer>backward = new HashSet<>();
			for(int i = 0;i<n;i++){
				if(c[i] == '(')stack.push(i);
				else if(!stack.isEmpty()){
					forward.add(stack.pop());
					forward.add(i);
				}
			}
			stack.clear();
			...
			if(forward.size() == n || backward.size() == n){
				...
				stack.clear();
				for(int i =n-1;i>=0;i--){
					if(forward.contains(i))continue;
					if(c[i] == '(')stack.push(i);
					else if(!stack.isEmpty()){
						backward.add(stack.pop());
						backward.add(i);
				...
				if(forward.size()+backward.size() == n){
					int res[]  = new int[n];
					sb.append(2+"\n");
					for(int it:forward)res[it] = 1;
					for(int it:backward)res[it] = 2;
