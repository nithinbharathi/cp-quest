...
public class Problem {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			for(int i = 0;i<n && pos.isEmpty();i++){
				int b[]  = Arrays.copyOf(a[i], m);
				t.sort(b);
				for(int j = 0;j<m;j++){
					if(a[i][j] != b[j])pos.add(j);
				}
			}
			if(pos.size()>2)sb.append(-1+"\n");
			else if(pos.size() == 0)sb.append(1+" "+1+"\n");
			else {
				swap(a,pos.get(0),pos.get(1));
				
				if(check(a))sb.append((pos.get(0)+1)+" "+(pos.get(1)+1)+"\n");
				else sb.append("-1\n");
			}
	...
	public static void swap(int a[][],int col1, int col2){
