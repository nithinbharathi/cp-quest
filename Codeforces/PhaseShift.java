...
/**
 * @submission : Nithin Bharathi 15-Sept-2023
 *
 *
 */
...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++) {
				int child = c[i] - 'a';
				if(map.containsKey(child))sb.append((char)('a'+map.get(child)));
				else {
					int parent = getParent(child,graph); 
					graph[parent] = child;
	...
	public static int getParent(int child, int graph[]) {
		int par = -1;
		for(int i = 0;i<26;i++) {
			if(graph[i] == -1) {
				cnt = 0;
				if(reachable(child,i,graph)) {
					if(cnt == 25) {
						par = i;
						break;
					}
	...
	public static boolean reachable(int child, int par,int graph[]) {
		while(graph[child] != -1) {
			child = graph[child];cnt++;
		}
