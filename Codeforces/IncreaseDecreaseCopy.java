	...
	public class Problem {
		...
		public static void main(String[] args) {
			...
			while (test-- > 0) {
				...
				long ans = 0, closest = Integer.MAX_VALUE;
				for(int i = 0;i<n;i++) {
					ans+= Math.abs(a[i]-b[i]);
					closest = Math.min(closest, Math.abs(a[i] - b[n]));
					closest = Math.min(closest, Math.abs(b[i]-b[n]));
					if(Math.min(a[i], b[i])<=b[n] && Math.max(a[i],b[i])>=b[n])
						closest = 0;
					
				}
				
				sb.append((ans+closest+1)+"\n");
