...
public class Problem {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			...
			int onlyInA = 0, onlyInB = 0,both = 0;
			for(int i = 1;i<=k;i++) {
				if(a.containsKey(i) && !b.containsKey(i))onlyInA++;
				else if(b.containsKey(i) && !a.containsKey(i))onlyInB++;
				else if(a.containsKey(i) && b.containsKey(i))both++;
			}
			boolean ok = false;
			if(onlyInA<=k/2 && onlyInB<=k/2) {
				int rem = (k/2 - onlyInA) + (k/2 - onlyInB);
				if(rem<=both)ok = true;
			}
			sb.append(ok?"YES\n":"NO\n");
