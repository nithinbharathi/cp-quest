...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			...
			for(int i = 0;i<n;i++)a[i] = t.readLong();
			
			if(k>=3)sb.append(0+"\n");
			else {
				long min = Long.MAX_VALUE;
				
				for(long it:a)min = Math.min(it,min);
				...
				if(k>=2) {		
					Collections.sort(l);
					for(int i = 0;i<n;i++) {
						int ind = search(l,a[i]);
						min = Math.min(Math.abs(l.get(ind) - a[i]),min);
						if(ind+1<l.size()) min =Math.min(Math.abs(l.get(ind+1)-a[i]),min);
						if(ind -1>=0) min = Math.min(Math.abs(l.get(ind-1)-a[i]),min);
					}
				}
