...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			int k = t.readInt();
			int cnt[] = new int[26];
			for(char c:t.read().toCharArray())cnt[c-'a']++;
			int ans = 1,l =2,r = n;
			while(l<=r){
				int mid = (l+r)/2;
				if(isPossible(cnt,mid,k)){
					ans = mid;
					l = mid+1;
				}else r= mid-1;
			}
	...
	public static boolean isPossible(int cnt[], int tar,int k){
		long pairs = 0,rem=0;
		for(int i = 0;i<26;i++){
			pairs+=(cnt[i]/2);
			rem+=(cnt[i]%2);
		...
		pairs*=2;
		if(tar%2==0)return pairs/tar>=k;
		tar--;
		long have = pairs/tar;
		long min = Math.min(k,have);
		rem+=(pairs-(min*tar));
		long ans = Math.min(min,rem);
		return ans>=k;
	}
