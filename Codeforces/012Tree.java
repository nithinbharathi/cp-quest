...
public class Problem {
	...
	public static void main(String[] args) {
	    ...
	    while(test-->0) {
	    	...
	    	if(a+1 != c)
	    		ok = false;
	    	int vertices = a+b+c-1;
	    	int height = 0;
	    	int curNodes = 1;
	    	if(ok) {
	    		while (vertices >=2 && a>0) {
					int add = 0;
					for (int i = 0; i < curNodes && vertices >= 2 && a > 0; i++) {
						vertices -= 2;
						a -= 1;
						add += 2;
					}
					...
					if(curNodes*2 != add) {
						curNodes= curNodes - add/2;
						add += Math.min(curNodes, b);
						b-= Math.min(b, curNodes);
					}
					curNodes = add;
					height++;
				}
			...
			height += (b + (curNodes - 1)) / curNodes;
			sb.append((ok?height:-1) + "\n");
		}
