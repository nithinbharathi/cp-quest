...
private static long go(int n, int k) {
	...
	while(l<=r) {
		...
		long prefixSum = sum(k,mid);
		long suffixSum = sum(mid+1,n+k-1);
	...
	long prefixSum1 = sum(k,ind);
	long suffixSum1 = sum(ind+1,n+k-1);
	
	long prefixSum2 = sum(k,ind-1);
	long suffixSum2 = sum(ind,n+k-1);
	
	return Math.min(prefixSum1-suffixSum1, suffixSum2-prefixSum2);
}
