...
/**
 * @submission : Nithin Bharathi 27-June-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++){
				a[i] = t.readInt();
				if(a[i]%2 != a[0]%2 && i>0)ind = i;
			}
			if(ind == -1){
				for(int i = 0;i<n-1;i++)l.add(new Pair(i+1,n));
			}else{
				 if(a[0]%2 == a[n-1]%2){
					 a[n-1] = a[ind];
					 l.add(new Pair(ind+1,n));
				 }
				 for(int i = 1;i<n;i++){
						if(a[i] %2 != a[0]%2)l.add(new Pair(1,i+1));				
					}
					for(int i = 1;i<n;i++){
						if(a[i] %2 == a[0]%2)l.add(new Pair(i+1,n));				
					}
			 }
