	...
	public class Problem {
		...
		public static void main(String[] args) {
			...
			while(test-->0) {
				char s[] = t.read().toCharArray();
				char c[] = t.read().toCharArray();
				int n = s.length;
				HashMap<Character,TreeSet<Integer>>map = new HashMap<>();
				for(int i = 0;i<n;i++) {
					TreeSet<Integer>set = map.getOrDefault(s[i], new TreeSet<>());
					set.add(i);
					map.put(s[i], set);
				}
				int m = c.length;
				...
				boolean ok = true;
				for(int i = 0;i<m;i++) {
					TreeSet<Integer>set1 = map.getOrDefault(c[i], new TreeSet<>());
					TreeSet<Integer>set2 = map.getOrDefault('?', new TreeSet<>());
					Integer first = set1.ceiling(pos);
					Integer sec = set2.ceiling(pos);
					if(first == null)
						first = n;
					if(sec == null)
						sec = n;
				...
				if(ok) {
					sb.append("YES\n");
					for(char it:s)sb.append(it=='?'?'a':it);
				}else sb.append("NO");
				sb.append("\n");
			}	
