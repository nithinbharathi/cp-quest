...
/**
 * @submission : Nithin Bharathi 22-Sept-2023
 *
 *
 */
...
public class ProblemB {
	...
	public static boolean go(int val, long pref[],long fact[],int k) {
		long ans = 0;
		for(int i = 0;i+val-1<pref.length;i++) {
			long sum = pref[i+val-1] - (i-1>=0?pref[i-1]:0);
			long factors =fact[i+val-1]-fact[i];
			if(sum<=k && factors == val-1) return true;
