...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			int n = c.length;
			TreeSet<Integer>set[] = new TreeSet[11];
			for(int i =0;i<=10;i++)set[i] = new TreeSet<>();
			for(int i = 0;i<n;i++){
				int dig = c[i]-'0';
				set[dig].add(i);
			}
			n = t.readInt();
			char left[] = t.read().toCharArray();
			char right[] = t.read().toCharArray();
			int l = left[0]-'0';
			int r = right[0]-'0';
			boolean ok = false;
			for(int i = l;i<=r;i++){
				Integer index = set[i].ceiling(0);
				if(index == null){
					ok = true;
					break;
				}
				for(int j = 1;j<n;j++){
					int maxIndex = -1;
					for(int k = left[j]-'0';k<=right[j]-'0';k++){
						Integer pos = set[k].ceiling(index+1);
						if(pos != null)maxIndex = Math.max(maxIndex, pos);
						else {
							maxIndex = -1;
							break;
						}
	...
	public static ArrayList<Long> getFactors(int n){
