...
/**
 * @submission : Nithin Bharathi 29-May-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i =0;i<k;i++){
			long val = res.get(i);
			if(cur+val<=n)cur+=res.get(i);
			else if(cur-val>=1)cur-=res.get(i);
			else ok = false;
			l.add(cur);		
			if(i-1>=0 && l.get(i).equals(l.get(i-1)))ok = false;
		}
		
		if(ok){
			System.out.println("YES");
			for(long it:l)System.out.print(it+" ");
	...
	static class Pair{
		...
		long lx,ly;
		boolean z;
		public Pair(int a,int y,int sz){
			this.x = a;
			this.y = y;
			this.sz = sz;
		}
