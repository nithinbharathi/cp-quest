...
/**
 * @submission : Nithin Bharathi 21-May-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			int n = t.readInt();
			graph = new LinkedHashSet[n];
			for(int i = 0;i<n;i++)graph[i] = new LinkedHashSet<>();
			for(int i = 0;i<n;i++){
				int val = t.readInt()-1;
				graph[i].add(val);
				graph[val].add(i);
			...
			long bamboo = 0,cycle = 0;
			for(int i = 0;i<n;i++){
				if(!vis[i]){
					boolean ok = dfs(vis,i,-1);
					if(ok)cycle++;
					else bamboo++;
			...
			long min = cycle+Math.min(1, bamboo),max = cycle+bamboo;
			sb.append(min+" "+max+"\n");;
...
public static boolean dfs(boolean vis[], int node, int par){
	vis[node] = true;
	boolean ok = false;
