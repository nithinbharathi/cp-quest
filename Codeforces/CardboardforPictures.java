...
public class ProblemC {
	...
	public static long getCount(long n,HashMap<Long,Long>map){
		long cnt = 0;
		for(long i = 1;i*i<=n;i++){
			if(n%i == 0){
			//	System.out.println(n+" "+i+" "+map);
				long first = i;
				cnt+=map.getOrDefault(first,0l);
				if(n/i != i)cnt+= map.getOrDefault(n/i,0l);
			}
		}
		return cnt;
	}
