...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++)l.add(t.readLong());
			Collections.sort(l);
			long left =0,right = (int)1e9;
			long ans = -1;
			while(left<=right){
	...
	public static boolean ok(long tar,ArrayList<Long>l){
		int ind = 0,cnt = 0;
		for(int loop = 0;loop<3;loop++){
			for(int i = l.size()-1;i>=ind;i--){
				long diff = Math.abs(l.get(i)-tar);
				if(Math.abs(l.get(ind)-diff)<=tar){
					ind=i+1;
					break;
				}
			}
