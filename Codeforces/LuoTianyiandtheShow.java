...
/**
 * 
 * @submission : Nithin Bharathi 11-June-2023
 * 
 */
...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			for(int i = 1;i<=m;i++){
				if(pos.contains(i))pref[i] = 1;
				if(i-1>=1)pref[i]+=pref[i-1];
			}

			long ans = Math.max(Math.min(m, pref[m]+right), Math.min(m,pref[m]+left));
			for(int i:pos){
				long  rs = Math.min(m-i, (pref[m]-pref[i])+right);
				long ls = Math.min(i-1, (pref[i-1]-pref[0])+left);				
				long cur = 1+rs+ls;
