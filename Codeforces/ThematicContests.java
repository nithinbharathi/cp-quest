	...
	public class Problem {
		...
		public static long go(long st,ArrayList<Pair>l){
			long cnt = 0;
			int ind =0;
			while(true){
				int index = find(ind,l,st);
				if(index == -1)break;
				cnt+=st;
				ind = index+1;
				st*=2;
		...
		public static int find(int ind,ArrayList<Pair>l,long target){
			int left = ind,right = l.size()-1;
			int index = -1;
			while(left<=right){
				int mid = (left+right)/2;
				if(l.get(mid).y>=target){
					index = mid;
					right = mid-1;
				}else left = mid+1;
			}
