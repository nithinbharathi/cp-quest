...
/**
 * @submission : Nithin Bharathi 03-July-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++){
				a[i] = t.readInt();
				if(a[i]>n || set.contains(a[i]))l.add(a[i]);
				else set.add(a[i]);
			}
			Collections.sort(l);
			boolean ok = true;
	...
	public static int go(int index,ArrayList<Integer>list,int val){
		int l = 0,r = index;
		int ind = -1;
		while(l<=r){
			int mid = (l+r)/2;
			if(list.get(mid)-val==0 || list.get(mid)%(list.get(mid)-val) == val){
				ind = mid;
			}
			l  = mid+1;
		}
