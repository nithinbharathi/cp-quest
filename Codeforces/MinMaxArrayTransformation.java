...
/**
 * @submission : Nithin Bharathi 19-July-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			...
			int ptr1 = 0, ptr2 = 0;
			for(int i = 0;i<n;i++){
				while(ptr1<n && (b[ptr1]<a[i] || a[ptr1]>b[i]))ptr1++;
				sb.append((b[ptr1]-a[i])+" ");
			...
			for(int i = 0;i<n;i++){
				int mx = b[i];
				while(ptr2<n && b[ptr2]>=a[i] && a[ptr2]<=mx){
					mx = Math.max(mx,b[ptr2]);
					ptr2++;
				}
				sb.append((b[Math.max(0, ptr2-1)]-a[i])+" ");
