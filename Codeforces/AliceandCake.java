...
/**
 * @submission : Nithin Bharathi 25-July-2023
 *
 *
 */
...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		int test = t.readInt();
		ArrayList<Pair>l[] = new ArrayList[N];
		...
		while(test-->0){
			...
			for(int i = 0;i<n;i++){
				...
				tree.put(sum, 1);
				int cnt = 0;
				while(!map.isEmpty() || !tree.isEmpty()){					
					long key = tree.lastKey();
					if(key<map.lastKey())break;
					else if(map.containsKey(key))remove(map,key);
					else{
						long first = key/2;
						long sec = key/2 + key%2;
						tree.put(first, tree.getOrDefault(first, 0)+1);
						tree.put(sec, tree.getOrDefault(sec, 0)+1);
					}
					remove(tree,key);
				}
			
		
			sb.append(map.isEmpty()?"YES\n":"NO\n");
	...
	public static void remove(Map<Long,Integer>map, long key){
