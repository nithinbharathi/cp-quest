...
public class Problem{
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
		    int n = t.readInt();
		    int a[] = new int[n];
		    long pref[][] = new long[n][31];
		    for(int i = 0;i<n;i++) {
		    	a[i] = t.readInt();
		    	for(int j = 0;j<31;j++) {
		    		if((a[i]&(1<<j)) != 0)pref[i][j]++;
	...
	public static int go(long pref[][], int val,int left) {
		int l = left, r  = pref.length-1,ind = -2;
		while(l<=r) {
			int mid = (l+r)>>1;
			long res = 0;
			for(int i = 0;i<31;i++) {
				long cnt = pref[mid][i] - (left-1>=0?pref[left-1][i]:0);
				if(cnt == mid-left+1) res+=(1<<i);
			}
