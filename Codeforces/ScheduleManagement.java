...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
		    ...
		    int m = t.readInt();
		    int cnt[] = new int[n];
		    for(int i = 0;i<m;i++)cnt[t.readInt()-1]++;
		    int l = 1,r = 500000,ans = -1;
		    while(l<=r) {
		    	int mid = (l+r)>>1;
		    	if(check(mid,cnt)) {
		    		ans = mid;
	...
	public static boolean check(int mid,int cnt[]) {
		long extraWork = 0,remWork=0;
		for(int i = 0;i<cnt.length;i++) {
			if(mid>=cnt[i])extraWork+=(mid-cnt[i])/2;
			else remWork+=(cnt[i]-mid);
			
		}
		return remWork<=extraWork;
	}
