...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			int n = t.readInt();
			char a[] = t.read().toCharArray();
			char b[]  = t.read().toCharArray();
			boolean single = a[0]!=b[0],both = a[0] == b[0];
			for(int i = 0;i<n;i++) {
				single &= (a[i]!=b[i]);
				both &= (a[i] == b[i]);
			}
			if(((single && !both) || (!single && both))) {
				ArrayList<Pair>l = new ArrayList<>();
				for(int i =0;i<n;i++) {
					if(a[i] == '0')l.add(new Pair(i+1,i+1));
				}
				if(l.size()>0) {
					char last = b[l.get(l.size()-1).x-1];
					if((l.size()-1)%2 == 1)last = last == '0'?'1':'0';
					if(last == '0')both = false;
					else both = true;
				}				
				if(both) {
					l.add(new Pair(1,1));
					l.add(new Pair(2,n));
				}else l.add(new Pair(1,n));
