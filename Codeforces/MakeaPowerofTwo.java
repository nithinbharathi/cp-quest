...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		int test = t.readInt();
		long powers[] = new long[63];
		powers[0] = 1;
		for(int i = 1;i<63;i++)powers[i] = powers[i-1]*2;
		while(test-->0){
			int n   = t.readInt();
			int ans = Integer.MAX_VALUE;
			for(long it:powers){
				char tar[] = String.valueOf(it).toCharArray();
				char cand[] = String.valueOf(n).toCharArray();
				int len1 = cand.length;
				int len2 = tar.length;
				int itr = 0, cnt = 0;
				for(int i = 0;i<len1;i++){
					if((itr<len2 && cand[i] != tar[itr]) || (i>=len2 && itr >= len2))cnt++;
					else if(itr<len2 && cand[i] == tar[itr])itr++;
				}
				ans = Math.min(ans, cnt+(len2-itr));
