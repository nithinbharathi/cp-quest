...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			long l = t.readLong();
			long r = t.readLong();
			long stFactor = search(l);
			long seFactor = search(r);
			long ans = Math.max(0,seFactor-stFactor-1)*3;
			for(int i = 0;i<=2;i++) {
				if(stFactor*(stFactor+i)<=r && stFactor*(stFactor+i)>=l)ans++;
				if(seFactor*(seFactor+i)>=l && seFactor*(seFactor+i)<=r && stFactor != seFactor)ans++;
			}
			sb.append(ans+"\n");
