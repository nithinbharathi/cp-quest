...
/**
 * @submission : Nithin Bharathi 04-June-2023
 *
 *
 */
...
public class Problem {
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 0;i<n-1;i++){
			int val  = t.readInt();
			graph[val].add(ind);graph[ind].add(val);
			ind++;
		}
		...
		while(q-->0){
			...
			int l_ind = map.get(u);
			int ans =-1;
			if(sizes.get(u)+1>=index)ans = l.get(l_ind+(index-1));
			sb.append(ans+"\n");
		}
		System.out.println(sb);
		
	}
	public static int dfs(int root,HashMap<Integer,Integer>sizes){
		int children = 0;
		l.add(root);
		vis[root] = true;
		for(int it:graph[root]){
			if(!vis[it]){
				...
				children+= 1+dfs(it,sizes);
			}
		}
		sizes.put(root,children);
