	...
	public class Problem {
		...
		public static void main(String[] args) {
			...
			while (test-- > 0) {
				...
				for(int i = 0;i<n;i++)
					a[i] = t.readInt();
				for(int i = 0;i<n-1;i++)
					gcd.add(t.gcd(a[i], a[i+1]));
				
				boolean ok = true;
				for(int i = 0;i<n-2;i++) {
					if(gcd.get(i)>gcd.get(i+1)) {
						ok = remove(i,a) || remove(i+1,a) || remove(i+2,a);
						break;
					}
				}
		...
		private static boolean remove(int ind, int a[]) {
			boolean ok = true;
			...
			for(int i = 0;i<a.length-1;i++) {
				if(i == ind)
					...
					if(i+2 < a.length) {
						l.add(t.gcd(a[i], a[i+2]));
					}
				}else
					l.add(t.gcd(a[i], a[i+1]));
			}
			for(int i = 0;i<l.size()-1;i++) {
				ok&=l.get(i)<=l.get(i+1);
			}
			return ok;
		}
