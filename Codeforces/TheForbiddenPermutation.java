...
/**
 * 
 * @submission : Nithin Bharathi 18-June-2023
 * 
 */
...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception{
		...
		while(test-->0){
			...
			int d = t.readInt();
			for(int i = 0;i<n;i++){
				a[i] = t.readInt()-1;pos[a[i]] = i;
			}
			int b[] = new int[m];
			for(int i = 0;i<m;i++)b[i] = t.readInt()-1;
			int ans = Integer.MAX_VALUE;
			for(int i = 0;i<m-1;i++){
				if(pos[b[i+1]]<pos[b[i]] || pos[b[i+1]]>pos[b[i]]+d)ans = 0;
				else{
					int case1 = pos[b[i+1]]-pos[b[i]];
					int req = ((pos[b[i]]+ d) - pos[b[i+1]])+1;
					int case2 = Integer.MAX_VALUE;
					int have = pos[b[i]]+(n-pos[b[i+1]]-1);
					if(have>=req)case2 = req;
