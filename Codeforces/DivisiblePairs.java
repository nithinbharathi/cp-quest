...
public class Problem {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0) {
			...
			int a[] = new int[n];
			HashMap<String,Integer>xys = new HashMap<>();
			long ans = 0;
			for(int i = 0;i<n;i++) {
				 a[i] = t.readInt();
				  String tt = ((x - a[i]%x)%x)+"$"+(a[i]%y);
				  ans += xys.getOrDefault(tt,0);
				  xys.put((a[i]%x)+"$"+(a[i]%y),xys.getOrDefault((a[i]%x)+"$"+(a[i]%y),0)+1);
			}
	...
	private static int go(int l, int r, int tar ,int a[]) {
		int ans = 0;
		if(l<=r) {
			ans = r - l+1;
			if(a[l] == tar)ans--;
