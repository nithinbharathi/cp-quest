...
public class ProblemB {
	...
	public static void main(String[] args) throws Exception {
		...
		while(test-->0){
			...
			long negmin = Integer.MAX_VALUE,posmin =Integer.MAX_VALUE;
			for(int i = 0;i<n;i++){
				a[i] = t.readInt();
				if(a[i] == 0)zero++;
				if(a[i] >0){
					pos++;
					posmin = Math.min(a[i], posmin);
				}
				if(a[i]<0){
					neg++;
					negmin = Math.min(a[i], negmin);
				}
				map.put(a[i], 1);
			}
			boolean ok = false;
			...
			if(zero == n)ok = true;
				...
				if(pos ==0 || neg == 0)ok = true;
				else if(-posmin == negmin)ok = true;
			}
			sb.append(ok?"YES\n":"NO\n");
